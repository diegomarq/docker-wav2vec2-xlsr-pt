...
### Diego
# Check if the file without ext in base_path corresponds to the file without ext specified in column name 
# python3 check_midia_name_ext.py cv-valid-test.csv audio midia_kaldi_base
...
def check_file(base_path, name, all_files):
    ...
    for f in all_files:
        if os.path.splitext(f)[0] in name:
